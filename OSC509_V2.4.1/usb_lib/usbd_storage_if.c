...
{
  /* USER CODE BEGIN 2 */
  /* SD card already initialized by FATFS (f_mount), just check state */
  if (BSP_SD_GetCardState() == SD_TRANSFER_OK)
  {
    return (USBD_OK);
  }
  /* fallback: try full init if card not ready */
...
{
  /* USER CODE BEGIN 6 */
  /* Ensure buf is word-aligned for SDMMC IDMA; if not, use internal buffer */
